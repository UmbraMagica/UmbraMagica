...
export default function CharacterEditNew() {
  const { user } = useAuth();
  ...
  const userForm = useForm<CharacterEditForm>({
  ...
  const adminForm = useForm<CharacterAdminEditForm>({
  ...
  const updateCharacterMutation = useMutation({
    mutationFn: async (data: CharacterEditForm | CharacterAdminEditForm) => {
  ...
  const onSubmit = (data: CharacterEditForm | CharacterAdminEditForm) => {
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent className="text-center">
                <div className="w-24 h-24 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full mx-auto mb-4 flex items-center justify-center text-white font-bold text-2xl">
                  {currentValues.firstName?.[0] || "?"}{currentValues.lastName?.[0] || "?"}
                </div>
                <h3 className="font-semibold text-lg text-foreground mb-2">
                  {currentValues.firstName || "Jméno"} {currentValues.middleName && `${currentValues.middleName} `}{currentValues.lastName || "Příjmení"}
                </h3>
                <p className="text-muted-foreground text-sm mb-3">
                  {currentValues.birthDate ? 
                    `${calculateGameAge(currentValues.birthDate)} let (rok 1926)` : 
                    "Věk nezadán"
                  }
                </p>
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent>
                <form onSubmit={currentForm.handleSubmit(onSubmit)} className="space-y-6">
                      ...
                      <div className="space-y-2">
                        ...
                        {currentValues.birthDate && (
                          <p className="text-xs text-muted-foreground">
                            Věk v roce 1926: {calculateGameAge(currentValues.birthDate)} let
                          </p>
                        )}
                      </div>
                  ...
                  {/* Regular fields available to all users */}
                  <div className="space-y-2">
                    <Label htmlFor="school" className="text-foreground">Škola</Label>
                    <Input
                      id="school"
                      {...currentForm.register("school")}
                    ...
                    <div className="bg-muted/50 rounded-lg p-4 border border-border">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <p className="text-foreground">{currentValues.firstName} {currentValues.middleName} {currentValues.lastName}</p>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Věk:</span>
                          <p className="text-foreground">
                            {currentValues.birthDate ? `${calculateGameAge(currentValues.birthDate)} let` : "Nezadán"}
                          </p>
                        </div>
                      </div>
                    </div>
